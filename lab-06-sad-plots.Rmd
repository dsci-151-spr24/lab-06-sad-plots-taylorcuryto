...
```{r faculty, fig.width = 10, fig.fullwidth = TRUE}
staff_long %>%
...
```{r faculty_named, fig.width = 10, fig.fullwidth = TRUE}
staff_long %>%
...
***You can highlight the Part-time faculty line to emphasize the difference - the line increases.***

3.  Implement the changes you proposed in the previous exercise.

```{r part_time, fig.width = 10, fig.fullwidth = TRUE}
staff_long %>%
  ggplot(aes(x = year, y = value, group = faculty_type, color = faculty_type)) +
  geom_line() + 
  labs(
...
***We can make the capture vs agriculture from each place a proportion of each other. We can put all this data on one bar chart graph and make it look pretty with a nice key. We can also look at the countries that have the fisheries total above 1000000 to only look at ones that have a lot of numbers.***
